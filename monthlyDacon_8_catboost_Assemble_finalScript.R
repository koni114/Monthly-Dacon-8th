...
#########################
if(finalVarBoolean){
  trainData <- train[c(finalVar_2, 'voted')]
  testData  <- test[c(finalVar_2)]
}else{
  trainData <- train
  testData  <- test
}
...
# catboost importance
catboost_imp           <- data.frame(model_2$feature_importances)
catboost_imp$variables <- rownames(model_2$feature_importances)
colnames(catboost_imp) <- c("importance", 'variables')
catboost_imp           <- catboost_imp %>% arrange(-importance)

finalVar_2 <- catboost_imp$variables[1:70]

##################################
## 버전별 최종 모델 & 변수 저장 ##
##################################
save(model_2, file = paste0("catBoost_model_2_", ver,".RData"))
save(finalVar_2, file = paste0("catBoost_finalVar2_", ver,".RData"))
...
## final assemble ##
####################
sample_submission$voted <- (YHat_cat + YHat_cat_2) / 2
